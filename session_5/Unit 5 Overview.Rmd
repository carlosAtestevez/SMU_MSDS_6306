...
Acu = read.csv("C:\\Users\\cestevez\\Dropbox\\Cloud PC\\Thinkpad\\Thinkpad Desktop\\Master Data Science SMU\\Class_Sessions\\Data Science Sessions\\Repository\\SMU_MSDS_6306\\session_5\\Acuspike Customer Data.csv",header = TRUE) # read in company data
...
```{r}
colors1 = c("\\borange\\b","\\bblue\\b","\\byellow\\b","\\bgreen\\b","\\bpurple\\b","\\bbrown\\b","\\bred\\b")
colors = c("orange","blue","yellow","green","purple","brown","red")
color_expression = str_c(colors1, collapse = "|")
color_expression
...
NYTIMES_KEY = "QPw9fgKWrpDrL9kS42X42UFXLTO1lJP2" #Your Key Here … get from NTY API website
...
articleStats[,c(2,3)] %>% gather(Type,Percent) %>% mutate(Word = rep(articleStats$Word,2)) %>% ggplot(aes(y = Percent, x = Type, fill = Word)) + geom_col() + facet_grid(~Word)+theme(axis.text.x=element_text(angle=60, hjust=1, size=10))
```
